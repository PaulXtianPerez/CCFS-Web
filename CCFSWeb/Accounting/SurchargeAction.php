...
if(isset($_POST['surcharge'])) {
  ...
  while($row = mysqli_fetch_array($result4)) {
    ...
    if(mysqli_query($conn, $query2) && mysqli_query($conn, $query3)) {
      echo "<span style='font-size:15px;'>" . '<i class="fas fa-check-circle"></i>' . " Surcharge added." . "</span>";
    } else {
      echo "<span style='font-size:15px;'>" . '<i class="fas fa-exclamation-circle"></i>' . " Failed to add surcharge." . "</span>";
    }
